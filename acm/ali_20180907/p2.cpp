...
为了保证分布式系统中的高可用特性，需要将每个分区复制多个副本(Replica)，分布放置到多台节点上。为了保证整个系统的每个节点负载是均衡的，需要保持每个节点上的所有分区的总数据大小大致均等。
...
现在假设数据一共划分了M个Partition，编号为(0,1,2…M-1)，同时有一个大小为M的数组partition_size_array描述每个分片的大小，比如[100,110,120,130]，表示P0.size=100,P1.size=110；
每个Partition有3个副本，现在随机分配到有N(N>3)个节点上，有一个整数二维数组node_replica_table(size=N)描述当前每个节点分配的Partition编号:
node_replica_table[0] = {0, 1, 5} ; // node0放置了partition0, partition1,
partition5; node_replica_table[1] = {1, 2, 3}; // node1放置了partition1,
...
内存限制: 128M (C/C++以外的语言为: 640 M) 输入: 输入数据包含N+2行： 第一行为 N M
D 第二行为 partition_size_array, 大小replica个数M(M>N)

...
struct node {
    VertexData data;            //表示u中距离此node最近的点的序号，范围[0, vexCounts-1]
    unsigned int lowestcost;    //最小代价，即此node到node data的距离。对于已经在v中的点，此值为0
} closedge[vexCounts]; //Prim算法中的辅助信息，closedge[i]表示结点i到u中结点data距离最短，为lowestcost
...
void MiniSpanTree_Prim(unsigned int adjMat[][vexCounts], VertexData s) {
    ...
    for (int e = 1; e <= vexCounts - 1; e++) {  //n-1条边时退出
        ...
        for (int i = 0; i < vexCounts; i++) { //更新v中顶点i的最小代价边信息

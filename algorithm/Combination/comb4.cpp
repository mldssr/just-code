 ...
 * 满足下式：
 *
 * 即C(n,m)模p等于p进制数上各位的C(ni,mi)模p的乘积。
 * 利用该定理，可以将计算较大的C(n,m)转化成计算各个较小的C(ni,mi)。
 * 该方案能支持整型范围内所有数的组合数计算，甚至支持64位整数，注意中途溢出处理。
 * 该算法的时间复杂度跟n几乎不相关了，可以认为算法复杂度在常数和对数之间。
 */
